...
class HomeController extends Controller
{
    ...
    {
        // Get all active categories with their active subcategories for sidebar
        $allCategories = AgricultureCategory::active()
        ...
        // Get categories that have products in stock (for other sections)
        ...
        foreach ($categories as $category) {
            $category->featured_products = AgricultureProduct::active()
                ->inStock()
        ...
        // Get categories for circular display (limit to 12 for horizontal scroll)
        $displayCategories = AgricultureCategory::active()
            ->whereHas('products', function($query) {
                $query->where('is_active', true)
                      ->where('in_stock', true);
            })
        ...
        return view('home', compact('allCategories', 'categories', 'displayCategories', 'featuredProducts', 'newArrivals', 'bestSellers', 'stats'));
    }
