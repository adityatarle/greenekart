...
<html lang="en">
  <head>
    <title>{{ $siteSettings['site_name'] ?? 'Greenleaf' }}</title>
    <meta charset="utf-8">
  ...
  <body>
    ...
    <header class="main-header">
      <div class="container-fluid px-3 px-lg-4">
        <div class="header-wrapper">
          ...
          <div class="header-search d-none d-md-flex">
            <form id="search-form" class="search-form" action="{{ route('agriculture.products.search') }}" method="GET">
              <div class="search-input-wrapper">
                <svg class="search-icon" width="20" height="20"><use xlink:href="#search"></use></svg>
          ...
          <nav class="header-nav d-none d-lg-flex">
            <ul class="nav-menu">
              <li class="nav-item">
                <a href="{{ route('agriculture.home') }}" class="nav-link {{ request()->routeIs('agriculture.home') ? 'active' : '' }}">
                  <span>Home</span>
                </a>
              </li>
              <li class="nav-item">
                <a href="{{ route('agriculture.products.index') }}" class="nav-link {{ request()->routeIs('agriculture.products.*') ? 'active' : '' }}">
                  <span>Shop</span>
                </a>
              </li>
              <li class="nav-item">
                <a href="{{ route('agriculture.categories.index') }}" class="nav-link {{ request()->routeIs('agriculture.categories.*') ? 'active' : '' }}">
          ...
          <div class="header-actions">
            <div class="action-icons">
              <a href="{{ auth()->check() ? (auth()->user()->isAdmin() ? route('admin.dashboard') : (auth()->user()->isDealer() ? route('dealer.dashboard') : route('customer.profile'))) : route('auth.login') }}" 
                 class="action-icon" 
                 title="{{ auth()->check() ? 'My Account' : 'Login' }}">
                <svg width="22" height="22"><use xlink:href="#user"></use></svg>
              </a>
              <a href="{{ auth()->check() ? route('agriculture.wishlist.index') : route('auth.login') }}" 
      ...
      <!-- Categories Sub Navigation - Only on Products and Categories pages -->
      @php
        $headerCategories = \App\Models\AgricultureCategory::where('is_active', true)->orderBy('name')->take(10)->get();
      ...
      <div class="categories-subnav d-none d-lg-block">
        <div class="container-fluid px-3 px-lg-4">
          <div class="categories-nav-wrapper">
            <button type="button" class="category-nav-btn category-nav-left" aria-label="Scroll categories left">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <polyline points="15 18 9 12 15 6"></polyline>
              </svg>
            </button>
            <div class="categories-scroll">
            <a href="{{ route('agriculture.products.index') }}" class="category-link {{ !request('category') && request()->routeIs('agriculture.products.index') ? 'active' : '' }}">
            ...
            @foreach($headerCategories as $cat)
              <a href="{{ route('agriculture.products.index', ['category' => $cat->id]) }}" 
                 class="category-link {{ request('category') == $cat->id ? 'active' : '' }}">
                {{ $cat->name }}
              </a>
            @endforeach
            @if(\App\Models\AgricultureCategory::where('is_active', true)->count() > 10)
              <a href="{{ route('agriculture.categories.index') }}" class="category-link view-all">
                View All
            ...
            <button type="button" class="category-nav-btn category-nav-right" aria-label="Scroll categories right">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <polyline points="9 18 15 12 9 6"></polyline>
              </svg>
            </button>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        var scrollEl = document.querySelector('.categories-scroll');
        var btnLeft = document.querySelector('.category-nav-left');
        var btnRight = document.querySelector('.category-nav-right');
        if (!scrollEl || !btnLeft || !btnRight) return;
        var scrollAmount = 200;
        function updateButtons() {
            btnLeft.disabled = scrollEl.scrollLeft <= 0;
            btnRight.disabled = scrollEl.scrollLeft >= scrollEl.scrollWidth - scrollEl.clientWidth - 2;
        }
