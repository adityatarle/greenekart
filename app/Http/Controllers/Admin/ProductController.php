...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Ensure all products have slugs (for route generation)
        // Set in memory first, then batch update if needed
        $productsToUpdate = [];
        foreach ($products as $product) {
            if (empty($product->slug) && $product->name && $product->exists) {
                $slug = \Illuminate\Support\Str::slug($product->name);
                $product->slug = $slug; // Set in memory for route generation
                $productsToUpdate[$product->id] = $slug;
            }
        }
        
        // Batch update slugs in database if any are missing
        if (!empty($productsToUpdate)) {
            foreach ($productsToUpdate as $id => $slug) {
                DB::table('agriculture_products')
                    ->where('id', $id)
                    ->whereNull('slug')
                    ->update(['slug' => $slug]);
            }
